...
def SolarSystem():
    ...
    for i in range(N):
        ...
        for k in range(100):
            ...
            # S1h = np.array([S1_help] * 9)

            S1 = np.ones((b, b, 3))
            S1[:] = S1_help

            S2 = np.transpose(S1, (1, 0, 2))
            S2 = S1.transpose((1, 0, 2))
            # S2 = np.transpose(S1,(3,2,1))
            S3 = S1 - S2
            S4 = []
            ...
            # print(S43[0])
            # S43H = np.linalg.norm(S42, axis=1)
            # print("###################")

            #S43 = np.linalg.norm(S42, axis=1)
            # print(S43[0])
            # time.sleep(10)
